...
const Home = observer(({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (state.isConnected) {
        const versionInfo = await axios.get('https://7fl5hvkwslbuefvbgfqx3cmwkq0eeuxg.lambda-url.eu-central-1.on.aws/version');
        ...
        if (await AsyncStorage.getItem('@version') !== versionInfo.data.version.toString()) {
          ...
          const response = await axios.get('https://7fl5hvkwslbuefvbgfqx3cmwkq0eeuxg.lambda-url.eu-central-1.on.aws/data');
  ...
  const setPressedCourses = crn => {
    ...
    for (let i = 0; i < data.courses.length; i++) {
      ...
      const CRNs = [];
      for (let j = 0; j < data.courses[i].classes.length; j++) {
        const cls = { sections: [], type: data.courses[i].classes[j].type };
        for (let k = 0; k < data.courses[i].classes[j].sections.length; k++) {
          const section = {
          ...
          for (let l = 0; l < data.courses[i].classes[j].sections[k].schedule.length; l++) {
            const schedule = {
